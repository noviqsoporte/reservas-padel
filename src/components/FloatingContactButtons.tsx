...
export function extractInstagramHandle(instagram: string): string {
    if (!instagram) return "";
    if (instagram.startsWith("http://") || instagram.startsWith("https://")) {
        const parts = instagram.replace(/\/$/, "").split("/");
...
export default function FloatingContactButtons({
    ...
    const waNumber = formatWhatsApp(telefono);
    const igUrl = buildInstagramUrl(instagram);

    // Stack: MapPin (bottom) → Instagram → WhatsApp (top)
    return (
        <>
            {/* WhatsApp */}
            <a
